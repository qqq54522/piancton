...
> 当前项目状态以 [图片搜索系统改造总纲](docs/IMAGE_SEARCH_REBUILD_MASTER_PLAN.md) 为唯一事实来源。Phase 0～6 工程改造已完成，当前已有 1 张代表素材并继续补充真实样本，Phase 0/4 真实数据验收仍为 `partial`。
...
- 一个业务搜索框、六大体系可选筛选、0～多个卖点意图识别；卖点公共话术先路由，accepted 卖点关系限定主通道，素材独有话术在通道内选图，数据库/Meilisearch/Embedding 多路召回只在无卖点素材或意图未消歧时兜底；结果顶部可按本次卖点缩小，卡片显示动态匹配卖点
