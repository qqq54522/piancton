...
2. 若 Prompt 中提供“候选体系卖点判断卡片”，优先按卡片的本体定义、核心对象、主动作、用户目的和排除边界判断；公共话术只能作为辅助入口，不能覆盖卡片边界。
3. 只返回输入中提供的当前启用卖点稳定 code，不得跨出候选体系，不得自造卖点。
4. 每个卖点必须有独立原话证据；共享入口词产生探索或待消歧候选，不得伪装成真多卖点。
5. 产品功能细节、教研方法、案例、数据、证书和效果材料属于第三层证明点，本层不得提前判断。
6. 显式否定的卖点写入 `excluded_concepts`，不得同时作为正向卖点。
