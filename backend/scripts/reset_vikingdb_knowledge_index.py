...
def reset(batch_size: int, dry_run: bool, async_write: bool) -> int:
    ...
    if not sync.client.configured and not dry_run:
        raise SystemExit("VikingDB 未配置或未启用，无法重置体系/卖点知识索引")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Delete all VikingDB documents, then rebuild only the 6 systems and "
            "16 selling-point knowledge documents."
        )
    )
    ...
    mode = "previewed" if args.dry_run else "reset and submitted"
