...
class SearchLog(Base):
    ...
    matched_concept: Mapped[Optional[str]] = mapped_column(String(200), nullable=True, index=True)
    top_image_ids_json: Mapped[str] = mapped_column(Text, default="[]")
    match_reasons_json: Mapped[str] = mapped_column(Text, default="[]")
    top_asset_group_ids_json: Mapped[str] = mapped_column(Text, default="[]")
    duration_ms: Mapped[Optional[int]] = mapped_column(Integer, nullable=True, index=True)
    timed_out: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    cache_hit: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    reranker_used: Mapped[bool] = mapped_column(Boolean, default=False, index=True)
    degraded_sources_json: Mapped[str] = mapped_column(Text, default="[]")
    branch_status_json: Mapped[str] = mapped_column(Text, default="[]")
