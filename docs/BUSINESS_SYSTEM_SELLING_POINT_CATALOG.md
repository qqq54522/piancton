...
版本：1.7
更新时间：2026-07-23
状态：六大体系 Skill 已接入项目实际搜索；共 16 个核心卖点，全部复用项目现有稳定 code；六个体系均已按“论断 + 搜索语言 + 证据样例 + 边界 + 当前素材线索”完成证明点第一版结构化，58 个绿色证据表达点均已保存结构化原图来源路径
...
- 证据表达点是原版截图中的绿色制作文字，用 `ep_` 稳定 code 保存父证明点、来源截图和可变深度 `sourcePaths`；它是人工审查和素材归因的来源锚点，不自动成为已核验产品事实。
- 业务负责人补充确认的跨卖点支撑关系单独保存在 `reviewNotes`，与 `sourcePaths` 严格分开：前者是人工校准说明，后者只能复现原图真实推导路径。
- 已审核图片通过 `expresses`、`supports`、`visual_related`、`excludes` 等关系连接卖点、证明点或证据表达点，不恢复固定二级标签树。
- 原图中间证明分支可保存在 `sourcePaths` 中用于溯源，不新增为数据库层级或普通搜索筛选项。
