...
当前上传和搜索不再调用图片语义分析；图片归属只依赖人工选择的卖点、渠道、场景/功能属性和审核关系。历史 AI 画面信息可以只读展示，但不得作为当前主流程的新写入来源。

- 六大体系稳定 code 以体系源文件和 `tags` 六个节点为准；可变化业务概念、概念搜索表达和素材关系以数据库概念层为准。
- 若未来重新启用任何 AI 写入任务，输出必须先经过 normalizer、schema 和 taxonomy 校验，再进入 pending/accepted/rejected 审核边界。
- 图片分析契约 Semantic Profile V3 目前仅作为历史兼容读取；不得在上传、替换主图或搜索中自动重新生成 OCR、主体、动作、视觉风格、可见功能、排除边界和客观 `content_tags`。
...
- 历史素材独有话术只保留兼容读取和审计；当前火山主链路不依赖公共话术或素材话术召回，新增上传也不再生成素材话术。
...
- `README.md`、`backend/README.md`、`ARCHITECTURE.md`、`SEARCH_MODES_AND_AI.md`、`API_CENTER_RELIABILITY_REQUIREMENTS.md`、`SERVER_DEPLOYMENT_CHECKLIST.md` 和 `UX_UI_DESIGN_SYSTEM.md` 是当前执行文档，必须与总纲一致。
