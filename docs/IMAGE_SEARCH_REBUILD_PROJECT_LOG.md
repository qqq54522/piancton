...
更新时间：2026-09-15
当前范围：Phase 0～Phase 6；D349 AI Search 单入口与 D350 Agent 自由问答/洋葱知识事实
当前状态：本地 AI Search 单入口和自由问答 Agent 已实施，知识与图片数据集已同时接入本地聊天；API 中心与额外模型调度不再参与当前运行时，旧 Agent 和渠道界面保留，云端未部署。下文 D330～D346 是回退前的历史实施记录，当前事实以总纲 D349/D350 为准。
