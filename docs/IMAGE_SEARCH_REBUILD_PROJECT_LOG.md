...
更新时间：2026-09-16
当前范围：Phase 0～Phase 6；D349～D371 AI Search 单入口、自由问答 Agent、渠道目录、账号体验与渠道结构复用
当前状态：D371 渠道分类结构复制工具已在本地完成并通过专项回归，云端未部署；D349～D366 已部署到云端，后续本地完成项的状态以总纲接力区为准。

> 本文档记录项目实际做过的工作、迁移、验证结果和遗留事项。架构原则、业务决策与后续阶段路线仍以 `docs/IMAGE_SEARCH_REBUILD_MASTER_PLAN.md` 为唯一事实来源。后续日志按日期追加，不覆盖历史记录。

---

## 2026-09-16：渠道分类结构复制工具（D371，本地完成）

- “场景与渠道管理”选中已有分类的来源渠道后，可打开“复制分类结构”，选择目标渠道并确认。系统按完整父子路径递归复制任意深度目录；目标路径已有同名目录时直接复用，其下缺少的子目录仍会补齐，重复执行保持幂等。
- 复制接口只处理 `channel_folders`。它不读取来源图片清单，也不新增、更新或删除 `image_channel_placements`，因此 PPT 的目录可以复用到官网大图，但 PPT 图片不会被带到官网大图目录，官网大图原有图片也不会被移动。
- 新增 `POST /api/channel-folders/folders/copy`、管理员/设计师审计动作 `channel.folder.copy_tree` 和管理页轻量操作区；沿用既有权限、渠道与目录模型，无数据库迁移，不改搜索、推荐、Agent、素材版本、六体系、人工 accepted 关系或火山数据集。
- 验证：后端渠道目录专项 3 项通过，覆盖部分同名结构合并、完整层级、图片绑定不变及重复执行；前端管理页专项与既有目录组件共 2 项通过；TypeScript、ESLint、生产构建、Ruff 和差异检查通过。Backend 已按当前代码重建，云端未部署。

---

## 2026-09-16：业务端每日昨日反馈（D367，本地完成）

- 业务用户从注册次日起，每个北京时间自然日第一次进入时自动出现“昨日使用反馈”强制弹窗；管理员、设计师不显示，新注册当天不要求反馈不存在的前一天，新人引导优先。
- 新弹窗复用原飞书 iframe 容器并使用用户提供的独立表单 `shrcnPICl1dbBjd0KXW5xUCHTFd`。关闭按钮、Escape 和点击背景均不能跳过；用户在飞书内提交后点击“我已提交，完成今日反馈”，后台才将当天写入账号。飞书跨域页面不能可靠把提交成功事件交给 Piancton，因此产品边界是用户显式确认，不伪造自动校验。
- 新增 `users.daily_feedback_completed_on`、迁移 `20260916_0043`、完成接口和 `dailyFeedbackRequired` 账号状态；同日刷新、重新登录或换设备不重复。原头像菜单中的手动项目反馈仍保留，搜索、推荐、Agent、六体系、人工关系、火山知识与图片数据集均未改变。
- 验证：后端完整回归 389 通过、4 跳过；前端 35 个文件共 95 项通过；TypeScript、ESLint、生产构建、Ruff 和差异检查通过。本地 PostgreSQL 迁移头为 `20260916_0043`。飞书地址用普通 GET 经访客登录跳转后最终返回 200，响应未声明 `X-Frame-Options` 或 CSP frame 限制。本地将当天注册的测试业务账号创建时间临时回拨一天后，浏览器实测显示标题、说明、飞书 iframe 和唯一完成按钮，随后已恢复原创建时间；管理员页不显示。云端未部署。

---

## 2026-09-16：新人 Agent 引导与账号头像（D366，本地完成）

- 首次使用弹窗移除“顶部搜索”旧文字，以高对比色问题示例展示六体系、同步课/培优课、渠道浏览、新疆合作案例找图及“先问卖点、再找图”的连续对话。例子只作引导，不作为 Agent 硬规则。
- 注册页提供六个预设账号头像及自定义图片选择；自定义文件登录后从项目已有私有存储保存，登录后账号菜单也可更换。新增 `users` 头像字段与迁移 `20260916_0042`，头像当前用户专用，不进入图库或 AI Search 索引。
- 验证：后端全量 387 通过、4 跳过；注册和头像专项 13 通过，覆盖私有读取、CSRF、预设切换、独立存储与图库不新增素材。前端全量 93 通过，包含新增自定义头像注册上传专项；TypeScript、ESLint、生产构建、Ruff 与差异检查通过。OpenAPI 类型已重新生成，本地 PostgreSQL 迁移头 `20260916_0042`，Backend/Web 健康。浏览器当前两页均为已登录账号，未中断其会话去模拟初次弹窗；云端未部署。

---

## 2026-09-16：Agent 卖点找图直接返素材卡（D365，本地完成）

- 现场业务账号连续询问“把这两个卖点的图片给我找出来”“我要的是素材卡”；火山 AI Search 生成适配图片名称，但本地两个助手消息的 `context_cards_json` 都为空，页面只显示文字清单。AI Search 所列名称与本地当前发布标题并不一致，不能直接把上游文字包装成可用本地卡。
- 明确按卖点找图时，读取当前请求或紧邻回答里确实出现的启用卖点名称；从既有业务概念仓库取人工 accepted `expresses`、current/published 图片，过滤测试占位图，并返回真实缩略图、身份码、图片详情与下载入口。普通解释保持自由问答；同名标题与渠道找图的现有核对优先。流式回答暂缓展示上游未核实的图片名单，最终将可核对的卡和简短说明一起写入同一会话。
- 修改 `backend/app/services/asset_agent_service.py`、`backend/tests/test_asset_agent_service.py`、总纲及本日志；没有迁移、旧 API 中心、知识/图片数据集、六体系或人工关系改写。新回归模拟 AI Search 只有虚构图片名称、没有图片 ID，检查同步与流式两种返卡、未审核图片排除和普通解释不出卡。
- Agent 专项 27 通过，完整 Python 3.12 后端 385 通过、4 跳过；改动文件 Ruff、差异检查通过。Backend 镜像已更新并健康；右侧业务账号现场追问后出现“拍题精学”“苏格拉底讲解提问”“举一反三”3 张可点击/下载的素材卡，点开第一张成功进入其详情页。先前只有文字的回答作为历史保留；云端尚未部署。

---

## 2026-09-16：Agent 同名素材卡与聊天续接（D364，本地完成）

- 现场症状：用户问“帮我找一张王玉龙专家的图片”只得到外部 IP 图片详情链接 `b87cd76e-...`，本地图库实际为“王玉龙专家”与“-01”两个不同当前图片 ID；接着问“之前聊了什么”时 Agent 声称零历史，右侧页面却连续显示两轮。只读数据库核对表明两轮分别写到了新会话，这是旧自然日清空后页面继续持有失效 ID、后台另建随机 ID 且前端未采纳返回 ID 所致；普通请求此前也没有显式带同一会话的文字问答。
- Backend：明确找并完整点名本地素材时，以当前已发布图片的精确展示标题族核对 AI Search 图片结果、补齐同主题现存尺寸版并返素材卡；覆盖掉未验证的外部图片详情链接与尺寸描述。普通追问把当前会话已保存的实际用户问题与可靠助手回答按序提供给 AI Search，不自加 24 条/12000 字等窗口；特殊“之前聊了什么”从该账号实际保存的聊天问题生成可核实清单，普通新对话不自动继承旧会话上下文。删除午夜自动删会话路径，既有 `expires_at` 非空字段保留为兼容而不再用于过期判断；合法旧会话 ID 若丢失可按相同 ID 重建，避免页面/后台连续分叉。前端在后台返回不同会话 ID 时采纳实际 ID。
- 修改：`backend/app/services/asset_agent_service.py`、`backend/app/repositories/image_repository.py`、`backend/app/repositories/asset_agent_repository.py`、`backend/tests/test_asset_agent_service.py`、`client/src/pages/ImageHome/AssetAgentWidget.tsx`、`client/src/pages/ImageHome/AssetAgentWidget.test.tsx`、总纲与日志。无迁移、无旧 API 中心或火山数据集改动。
- 验证：最终 Backend 384 通过、4 跳过；前端 91 项通过，TypeScript、ESLint、生产构建、Ruff、Python 编译和差异检查通过；本地 Backend/Web 均已重建并健康。保留用户原浏览器标签，在独立新版标签用当前预览账号实测：回顾回答列出实际保存的七条旧提问；再次找王玉龙专家，直接展示两张可打开和下载的当前素材卡，没有旧外部链接；只读数据库确认两问使用同一个会话 ID。
- 剩余：旧每日清空路径此前真实删除的消息无法从模型推断恢复，现有账号清单只能覆盖数据库仍保存的记录；云端未部署。项目旧 Pyright 检查仍有历史类型及旧向量文件问题，这次 Agent Service 改动没有新增报告。

## 2026-09-16：Agent 渠道归属找图（D363，本地完成）

- 核对已有业务交互：左侧分类图标位于渠道横向滚动区外；新图上传先选渠道，再可选该渠道任意深度的目录；业务账号头像位于顶部右侧，菜单含我的喜欢、收藏、信息、反馈和退出切换账号。无前端改动。
- 搜索链路：AI Search 保持唯一在线问答入口。明确找已创建渠道素材时，用本地 `managed_channels`、目录及当前已发布图片的真实渠道归属核对 AI Search 返回的 `item_ids`，过滤其他渠道并补齐本地结果；选中父目录时包含后代目录，多渠道图片按真实分隔边界匹配。流式渠道找图在核对后一次发准确文字及图片卡，普通问答继续自然流式输出。未知渠道或泛化问题仍由 AI Search 处理，没有添加业务硬标签、旧 API 调度或火山数据集变更。
- 自由问答边界：仅当明确有找素材动作时才使用渠道核对；“推荐 PPT 的制作方法”仍按普通聊天回答，专项回归已覆盖。
- 修改文件：`backend/app/repositories/image_repository.py`、`backend/app/services/asset_agent_service.py`、`backend/tests/test_asset_agent_service.py`，以及本总纲和日志。无迁移。新增回归用例在测试库模拟“合作案例/北京/朝阳/望京”与无关 PPT 卡；真实本地数据库目前只有 PPT、手机端大图两个管理渠道，没有合作案例图片，因而还不能现场实测这个示例。
- 验证：最终新版容器 Agent 专项 24/24、全量 Backend 382 通过/4 跳过；Ruff、编译、`git diff --check` 通过；Backend 已重建，容器健康且 `/health/ready` 返回 ready。Pyright 在本地 Python 3.9 环境仍有既存类型和旧向量文件错误；这次改动的 Agent Service 没有新增报告。前端未改，沿用 D362 的前端回归。
- 剩余：云端未部署；未来建立合作案例渠道、选择目录并确认图片归属后，需用真实素材复测。旧图没有被自动猜测归类；若没有匹配的已发布图片，Agent 会明确说本地渠道暂无匹配。

## 2026-09-16：Agent 连续问图与历史图片回指（D362，本地完成）

- 用户澄清目标是像普通聊天一样：“发一张图问问，再发另一张图问问，想怎么问就怎么问”，包括后来直接问“前两张发的是什么图片”。D361 只确保最新图是下一轮一次性附件，未确保旧图资料进入后续模型问题；D360 取消本地问答历史拼接后，该能力完全依赖火山 `session_id` 的平台记忆。临时上传图也没有写入服务端用户消息附件，刷新后连文件名无法回看。
- 后端每轮发送图库图片或临时上传图时，把标题/文件名写进该条用户消息图片附件；后续向同一火山会话提问时，提供此前发图的顺序、当时问题及已生成的可靠回答，便于自然语言回指。旧图不留在待发送栏，也不逐轮重复上传像素；旧问图失败会标明没有可靠画面回答。前端用户提问消息显示临时上传图的文件名附件。无迁移，不改素材搜索索引、六体系、人工业务关系、知识与图片数据集，也不恢复 API 中心。验证：后端 Agent 专项 23/23、完整回归 381 通过/4 跳过；前端 90/90、TypeScript、ESLint、生产构建、Python 编译和差异检查通过。Backend/Web 容器已更新并健康，应用浏览器已加载新版页面。云端未部署，真实视觉复测需要公网图片桥。

## 2026-09-15：Agent 待发送图片自动切换（D361，本地完成）

- 用户希望像普通 Agent 一样，聊完一张图后再从素材卡发另一张时，下一问自动使用新图。检查发现已成功问答后的上下文会清空，但上一张尚未提问或失败待重试时，前端会把新图追加到最多 8 张，违背“自动切换”。
- `AssetAgentWidget` 的素材卡发送改为下一条问题只保留最新一张图库图片；后台已有 context 更新会整组替换。已完成的图文聊天记录和火山同会话 `session_id` 均保持，失败后用户可再次选择原图。无迁移、不改视觉桥、素材、六体系、人工业务关系和火山知识/图片数据集。新增前端回归验证“先选 A、再选 B、下一问只发送 B”，并验证一次问答后清空附件、同会话继续追问；前端全量 89/89 通过，TypeScript、ESLint、生产构建与差异检查通过。Web 容器已重建并健康，更新后的本地页面已在应用浏览器打开。

## 2026-09-15：Agent 记忆展示与本地窗口简化（D360，本地完成）

- 用户指出“今日记忆”圆环达到 100% 不符合实际，并要求当前记忆不要受 Piancton 自定义条数/字数限制。调查确认百分比只来自本地按 24 条、单条 1200 字、总 12000 字的字符计算，与火山实际模型 Token/会话剩余容量无关；同日第 21 个 Agent 会话还会自动删除较早会话。
- 前端删除圆形进度与假百分比，Agent 会话 API 和前端模型移除 `memoryUsedChars`/`memoryLimitChars`/`memoryUsageRatio`。后端不再把旧问答剪裁后逐轮拼入 `chat_search` Prompt，仍为同一会话传同一个 `session_id`；删除同日 20 个会话限制，聊天记录仍可供用户查看。素材图片首次发送和成功后清除的行为保持。按上海时区每日清空是 D342 曾明确确认的记录保存规则，本轮暂保留，不把它误写成模型上下文容量。
- 同步修改 `backend/app/services/asset_agent_service.py`、`backend/app/repositories/asset_agent_repository.py`、`backend/app/schemas/asset_agent.py`、`backend/tests/test_asset_agent_service.py`、`client/src/pages/ImageHome/AssetAgentWidget.tsx`、`client/src/pages/ImageHome/assetAgentSessionModel.ts`、对应测试/类型、`docs/ARCHITECTURE.md`、`docs/UX_UI_DESIGN_SYSTEM.md`、总纲和本日志；无数据库迁移。官方 Viking AI Search 产品页公开说明使用豆包大模型系列和多轮能力，但未能从公开材料核实本应用具体型号、版本或 Token 窗口；不以任何豆包版本的 Token 上限代替本应用的实际容量。后端专项 26 项、完整 380 项通过/4 项跳过、前端 88 项、类型检查、ESLint、生产构建、Ruff、编译与差异检查通过。本地 Backend/Web 已重建，浏览器确认旧“今日记忆”圆环消失；云端未部署。

## 2026-09-15：Agent 图片一次性附件与连续追问（D359，本地完成）

- 核对现有实现：同一 Agent 对话调用火山 AI Search `chat_search` 时复用相同 `session_id`，并带入最多最近 24 条用户/助手消息、单条 1200 字和总计 12000 字的受控文本历史；当天会话持续，次日按上海时区清空。原图库图片却作为会话长期上下文逐轮重传，顶部必须手动清除。
- 改动：成功的图库视觉问答后后端清空待发送图片；用户提问消息保存该轮附图卡片，前端把缩略图与标题显示在消息中，顶部待发送栏随服务端会话状态移除。追问沿用相同 `session_id`、前一轮问答与附图标题历史，不再次提交视觉像素；若失败或视觉地址不可达，图片仍待发送供重试。临时上传附件沿用原有一次性行为。
- 修改：`backend/app/services/asset_agent_service.py`、`backend/tests/test_asset_agent_service.py`、`client/src/pages/ImageHome/AssetAgentWidget.tsx`、`client/src/pages/ImageHome/AssetAgentWidget.test.tsx`、`docs/UX_UI_DESIGN_SYSTEM.md`、总纲和本日志；无数据库迁移。不改六体系、卖点/证明点、人工 accepted 关系、素材和火山知识/图片数据集。专项后端 25 项、完整后端 379 项通过/4 项跳过、前端 89 项、类型检查、ESLint、生产构建、Ruff、编译和差异检查通过；本地 Backend/Web 重建并启动，浏览器确认新版待发送图片文案加载。
- 云端尚未部署；本地 `127.0.0.1` 没有火山可读取的公网图片地址，不能在本地页面完成真实视觉问答验收。云端成功看图行为由用户实测确认，D359 的一次性附件仍需部署后用真实图片验收。

## 2026-09-15：本地图库问图地址校验（D358，本地完成）

- 右侧浏览器当前会话已带入“押题合集-河北”，用户问“这张图片讲了什么”却只得到火山 AI Search 的泛化查找失败提示。运行时配置检查确认图库视觉桥地址回退到 `http://localhost`，没有显式 `AI_SEARCH_PUBLIC_BASE_URL`/`PUBLIC_BASE_URL`；火山云端不能从这个地址读取本机图片。
- `AssetAgentTemporaryImageService` 现在拒绝 localhost、回环和私有 IP 作为公网视觉桥。`AssetAgentService` 在图库图片画面不可供在线服务读取且用户询问图中内容时保留上下文并明确说明未看到画面；可由人工 accepted 事实支撑的“这张图怎么向家长讲”等问题继续走知识问答。未恢复旧 API 中心或额外 Provider，未改图片和知识数据集、人工关系与图片文件。
- 修改：`backend/app/services/asset_agent_temporary_image_service.py`、`backend/app/services/asset_agent_service.py`、`backend/tests/test_asset_agent_service.py`、总纲与本日志；无迁移。本地 Ruff/编译/diff 检查通过，Docker Agent/AI Search 专项 24 通过，完整后端 378 通过、4 跳过；后端镜像已重建并启动。右侧原会话及图片上下文仍在，旧泛化回答是保存的历史记录，没有自动篡改。
- 未完成的真实视觉验收需要将当前图片服务放到火山可读取的公网地址，显式配置 `AI_SEARCH_PUBLIC_BASE_URL` 指向该服务，并用相同图片重新提问验证火山看到像素。自动审批拒绝把当前图库图片额外发往火山测试内嵌图片格式；本轮未进行这项外发，也没有证据认定 AI Search `chat_search` 支持内嵌 Data URI。

## 2026-09-15：旧 API 调度与模型分析隔离清理（D357，本地完成）

- 当前 Agent 的路由依赖只构建 `VolcAiSearchClient`，直接使用应用的 `chat_search`，知识与图片数据集均由 `AI_SEARCH_CHAT_DATASET_IDS` 提供；旧 API 中心路由和页面已在 D349 下线。
- 移除后端 `app.ai` 包初始化对旧 Provider 工厂的导入，删除旧工厂、OpenAI 兼容适配器、备用调度、图片分析任务和旧 `AiService`。旧搜索编排的可选 AI 类型只作兼容协议，当前启动依赖仍明确传入 `None`；上传、版本和人工卖点关系写入时不再初始化或调用旧 Embedding/VikingDB 索引。
- 从 `Settings` 和本机忽略的三份环境配置中移除旧模型、备用模型、图片分析、Embedding/Reranker、VikingDB 与旧知识服务项，保留火山 AI Search 应用、知识与图片数据集配置。从正式 Skill 清单删除已退休的独立 Provider、图片分析与旧文案匹配 Skill，并改写架构 Skill 防止未来重接；历史迁移与旧数据库模型只为无损升级保留。
- 验收：Docker Python 3.12 后端全量 374 通过、4 跳过；本机 Ruff 相关文件、Python 编译、Compose 配置与差异检查通过。后端/Web 容器健康，启动导入检查显示旧 Provider、图片分析和 VikingDB 知识路由模块均未加载。旧七条 Phase 4 用例明确要求已退役的分层模型 API 调用，已删除；其余确定性/人工关系回归保留。云端未部署，历史迁移和旧数据库表未删除。

## 2026-09-15：Agent 问答停顿复核（D353，本地诊断完成）

- 用户指出火山 AI Search 以前应较稳定。回退基线 `ebebe8f` 的 Agent 在 AI Search 失败时还有独立 `ModelProvider` 问答兜底，旧页面成功率不等于 AI Search 单请求成功率；当前 AI Search HTTPX 流客户端与基线实现相同，未因 D349 改写传输方法。
- 同一火山应用和两数据集短问题：本机直连 27 秒、容器 20 秒成功，容器取 10 条约 23 秒成功；本机和容器的解析地址及数据集指纹一致。完整 Agent Prompt、最近会话与 10 条检索在容器中数次于 `get results` 后停住 35～40 秒，读流超时；本机同请求约 18 秒成功。现有样本不足以证明火山平台整体退化或 Docker 持续断网。
- 更换读取方式作对照：容器标准库 `urllib` 第一次约 20 秒成功、第二次同阶段约 40 秒超时；HTTPX 原始字节约 22 秒成功、解码字节同阶段约 44 秒超时。因此没有证据将故障单独归给 `iter_lines()`，不更换运行时客户端。当前最精确定位是完整重请求在“检索结果已返回、回复尚未开始”阶段偶发停顿；需要火山侧请求追踪或云端持续阶段指标，才能区分上游生成与本地流传输路径。
- 本轮只更新总纲与项目日志，没有代码和数据库迁移；D351/D352 的本地 45 项回归与浏览器真实问答验收仍有效。下次在云端部署后统计脱敏失败类型、耗时与阶段，若重请求仍重复停顿，再与火山控制台请求记录对照；保持 AI Search 单入口。

## 2026-09-15：Agent 问答检索量与传输重试（D352，本地完成）

- D351 将每段读取预算提高到 60 秒后，真实复测同一详细提问先在约 5 秒发生 TLS `ConnectError`，再次运行则在约 65 秒发生 `ReadTimeout`；仅调高超时不能解决所有失败。DNS/TLS 独立探测随后正常，表明连接断开具有间歇性。
- 同一 AI Search 场景简短问“同步规划体系是什么”、取 4 条约 19 秒得到完整答复；保留 Agent 完整 Prompt、改取 4 条约 30 秒得到 584 字讲解。默认取 10 条的请求此前未在 60 秒内完成，因此普通问答缩小检索量，找图和附图保持既有数量。
- 新增回答正文开始前的有限一次连接/读取故障重试；正文开始后不重试，避免重复文字。两次连接均失败的真实连续追问仍发生过，页面兜底改为区分连接中断与响应超时。
- 本地验收：重建 backend/web 后，浏览器新页面无 API 中心和旧“问题分析”卡片，Agent 欢迎语为自由问答版；在原会话直接重问“请详细讲解一下同步规划体系”，18:27:19 发送、18:27:40 保存完整讲解且 `used_model=True`。Agent/AI Search 专项 45 项通过，Ruff、Compose 配置及 diff 检查通过。
- 修改文件：`backend/app/services/asset_agent_service.py`、`backend/tests/test_asset_agent_service.py`、本总纲与项目日志；D351 同时调整 `backend/app/core/config.py`、`docker-compose.yml`、`.env.docker.example`、`backend/.env.example` 和本机忽略的 `.env` 配置。无数据库迁移。剩余问题是 AI Search 问答 POST 偶发连接断开或上游停顿仍不能保证每轮成功；下次上线前部署到云端并用真实流量记录失败类型和耗时。

## 2026-09-15：Agent 详细问答超时修复（D351，本地完成）

- 现象：同会话六体系问答成功，随后“请详细讲解一下同步规划体系”返回在线问答不可用。后台会话时间为 17:54:12 提问、17:54:32 失败；另一条“讲个故事”约 24 秒后失败，与当前 20 秒 AI Search 流式读取预算吻合，但旧服务没有错误日志，不能复原具体故障。
- 判断：保护接口、Nginx 和 AI Search 应用配置都可用；失败来自上游流式调用阶段，现有代码吞掉错误且无脱敏耗时日志，无法进一步区分第一段迟到和中途停顿。
- 实施与验收：聊天读取预算提高到 60 秒，记录不含用户问题、答案和密钥的错误类型与耗时，保持同一 AI Search 单入口；真实复问和 45 项回归结果见 D352。无数据库迁移。

## 2026-09-15：Agent 自由问答与洋葱知识事实（D350，本地完成）

- 用户截图：问“洋葱的六大体系是什么”，AI Search 聊天答成“AI 智能学伴体系、AI 未来课堂体系、智能学习机体系……”等外部公开口径；项目正式六体系实际上是同步校内、同步考点、同步培养、同步规划、同步自学、同步伴学。
- 根因：`AI_SEARCH_CHAT_DATASET_IDS` 为空时客户端只用图片数据集；旧 Agent 提示缺少六体系正式名称且包含过多卖点/找图/销售流程约束，前端又把每个回答包装成固定的物品检索“问题分析”。
- 实现：`AssetAgentService` 的 AI Search 聊天输入改为简短自由问答提示，动态读取项目稳定六体系目录作为轻量事实参考；连续追问时若旧助手答案与项目事实冲突，提示主动更正而不是沿用错误历史。删除未接入的旧本地 Agent Prompt 和固定业务兜底回答。Agent 欢迎语与前端默认问题覆盖普通写作、图片、找图和洋葱知识，不强制把任何问题转成卖点/销售话术。前端删除虚构的四步“问题分析”卡片，等待期间只显示简洁状态；失败时不再追加错误的“本地模型兜底”说明。
- 配置：用户提供知识数据集 ID 后，已写入本机 Git 忽略的根目录 `.env`，并把当前 AI Search 配置同步到 Git 忽略的 `backend/.env`，供 Docker 与后端直跑使用；知识与现有图片数据集一起送入 AI Search `chat_search`，运行中客户端确认同时包含两个数据集。未修改 Git 追踪的密钥或数据集配置，无数据库迁移。
- 验证：真实 AI Search 问答“洋葱的六大体系是什么”包含六个正式名称，未出现截图中错误的公开六体系；把错误答案放入上一轮历史再追问时，Agent 明确承认旧答案不准确并给出六个正确名称。普通“1+1 等于几”成功回答且未套入洋葱体系。另一次普通生日祝福写作探针超时，说明当前 20 秒上游聊天预算仍需稳定性观察。后端 Agent/AI Search 43 项，含 Skill/文档联合回归 47 passed、4 skipped；前端 88 项测试通过，TypeScript/生产构建、ESLint 和修改范围 Ruff 通过，本地 Docker 服务健康。
...
- 需求：用户明确素材库已有图片发送到 Agent 后也需要真实视觉分析，不能只依赖上传时保存的标题、图片摘要和人工业务关系。
- 实现：`AssetAgentService` 新增私有存储视觉桥。每次围绕素材库图片问答时，从本地或私有 TOS 读取压缩预览图，复用临时问图服务生成不可猜的短期 capability URL 交给火山 AI Search；首次发送和后续追问各生成新地址，响应结束立即删除。外部不再读取登录保护缩略图地址，也不获得原图、存储凭证或用户 Cookie。视觉桥失败时回退本地审核上下文，不阻断当前会话。
- 事实边界：火山视觉结果用于理解画面构成和可见信息，本地已审核摘要与人工 accepted 关系继续负责体系、卖点和适配事实；视觉猜测不能覆盖六大体系、16 卖点、图片标题或人工关系。本轮无迁移，不修改图片、渠道、对象存储、火山配置或数据集。
- 验证：新增素材库图片首次问图、连续追问均实际传入不同短期视觉 URL，并验证回答结束后两个 URL 均失效；后端联合专项 128 项通过，Ruff、Python 编译、Docker 后端生产构建和差异检查通过。

## 2026-09-14：部署登录态、Agent 图片上下文与同主题返图修复（D344）

- 现象与根因：后端重建后，浏览器里的旧图片和账号外观仍来自缓存，但渠道接口已经返回 `401`，旧界面把请求失败显示成“素材库为空”；素材库图片加入 Agent 后，火山问答被传入了需要 Piancton 登录态的缩略图地址，外部服务无法读取且同一会话每轮重复该地址；“学情报告”找图只展示火山首批少量候选，后续“其他几张”又可能扩展到同卖点下的“辅助学习”。
- 实现：保护接口 `401` 统一触发前端账号失效，清除查询缓存并回到登录页；渠道从手动卖点结果切换时回完整图库。素材库图片改用本地摘要、语义、素材组和人工卖点关系进入 Agent，临时上传图片的短期公开 URL 机制保持。Agent 以火山结果为高相关种子，用去除序号和常见尺寸后缀后的显示名称家族补齐本地 current/published 同主题版本；同主题续问读取上一轮推荐卡片并锁定主题，回答同步说明补齐数量和尺寸范围。
- 边界：名称家族只服务推荐展示完整性，不成为业务关系或搜索金标准；不修改图片标题、渠道、对象存储、六大体系、16 卖点、人工 accepted 关系、火山配置、行为闭环或三个既有数据集；无迁移。
- 验证：后端标题/Agent、AI Search 闭环、API 中心、渠道和注册专项共 128 项通过；前端完整 34 个测试文件共 90 项通过；TypeScript、ESLint、Ruff、生产构建和差异检查通过；本地浏览器真实复现并确认失效登录现在进入登录页，不再显示渠道空素材。

## 2026-09-11：火山 AI Search 首页搜索与素材同步（D304）

- 背景：用户希望首页顶部搜索框接入火山 AI Search，而不是继续只走本地/旧向量链路；同时素材由设计师在 Piancton 上传并人工选择渠道、卖点和业务关系后，应自动导入 AI Search 物品数据集，删除素材时同步删除外部索引数据。用户明确要求列表和外部搜索索引使用压缩预览图，只有进入详情才加载完整图。
- 实现：新增 `VolcAiSearchClient`、`VolcAiSearchService` 和 `VolcAiSearchIndexSync`。首页 `/api/images/search` 在无强业务过滤条件时优先调用火山 AI Search 应用搜索，再按返回的 `image_id/_id/id` 或 `identity_code` 回本地数据库水合成现有图片结果；AI Search 不返回本地存在的图片时不展示外部孤儿数据。上传、标题更新、筛选元数据更新、业务关系确认、素材恢复、软删除和物理删除均新增最佳努力同步：写入 `/api/v1/dataset/{dataset_id}/write` 的 `fields`，删除 `/api/v1/dataset/{dataset_id}/delete` 的 `_ids`。
- 数据字段：同步文档包含 `image_id`、`identity_code`、`title`、`channel`、`thumbnail_url`、`image_url`、`detail_url`、`updated_at`、`image_width`、`image_height`、`is_scene_image`、`status` 和聚合后的 `search_text`。其中 `image_url` 与 `thumbnail_url` 都指向本系统缩略图接口，`detail_url` 指向 `/image/{id}`；原图仍只由本系统详情页和下载流程按需加载。
- 配置：新增 `AI_SEARCH_ENABLED`、`AI_SEARCH_SYNC_ENABLED`、`AI_SEARCH_BASE_URL`、`AI_SEARCH_API_KEY`、`AI_SEARCH_APPLICATION_ID`、`AI_SEARCH_SEARCH_PATH`、`AI_SEARCH_DATASET_ID`、`AI_SEARCH_TIMEOUT_SECONDS`、`AI_SEARCH_PAGE_SIZE`、`AI_SEARCH_PUBLIC_BASE_URL` 和 `PUBLIC_BASE_URL` 示例项。真实 API Key 只放本机/服务器忽略提交配置，不进入代码和文档。
- 边界：外部 AI Search 是搜索候选索引，不替代本地 PostgreSQL 的素材事实、审核状态、详情、下载、身份码和人工 accepted 关系。外部搜索或同步失败只记录日志并回退旧搜索链路，不阻断上传、删除或人工业务事实保存。
- 验证：新增/改动的 AI Search 相关后端文件 Ruff 通过，Python 编译通过；`tests/test_search_service.py` 13 passed。`tests/test_security_and_images.py` 当前仍有 2 个失败，禁用 AI Search 后仍复现，属于当前工作树已有上传错误结构和旧统一搜索日志用例不匹配问题，不是本次外部 AI Search 接入引入；需要后续单独修正后再跑全量测试。

### 后续

- 云端部署时在服务器 `.env` 填入 AI Search 环境变量并重建 backend；用 3～5 张已上传素材验证写入、搜索、删除同步和预览图大小。
- 如果火山 AI Search 返回字段结构与当前解析有差异，优先在 `VolcAiSearchClient._extract_matches()` 中补兼容，而不是改前端卡片协议。
- 后续可把首页搜索结果页的诊断信息补充“AI Search 命中/回退”状态，方便运维判断当前是否真实走到新引擎。

## 2026-09-11：Piancton Agent 流式知识路由（D303）

- 背景：用户在火山知识库控制台验证到满意的“思考流式 + 最终结果”体验，并指出右侧 Agent 不能只靠通用模型回答，必须接入现有知识库与向量数据库卖点判断链路。
- 实现：新增 `POST /api/asset-agent/sessions/{session_id}/messages/stream`，返回 `text/event-stream`。后端先输出可见业务判断过程，再输出最终回答，并在结束时保存同一条会话消息；旧 `/messages` JSON 接口保留，前端流式失败时自动回退。`AssetAgentService` 注入搜索主链路同源的知识路由，优先调用火山知识库服务，知识库无可信卖点时走 VikingDB 向量知识兜底，并把裁决结果作为 `asset_agent_chat` 的事实输入。
- 前端：`AssetAgentWidget` 优先使用流式接口，实时更新同一条 Agent 消息；`reasoning_delta` 展示为“思考中/已深度思考”区域，`answer_delta` 展示为正式回答区域。会话仍沿用原有用户私有记录、每日清空和图片上下文机制。
- 边界：不新增数据库迁移，不改变搜索页主链路，不改图片召回排序，不写入业务事实；图片仍只能来自本地数据库人工 accepted 关系。当前流式为 Agent 层 SSE，底层模型 Provider 尚未全部升级为原生 token stream。
- 验证：后端相关文件 `py_compile` 通过；前端 `npm run typecheck` 通过；前端 `npm run lint` 通过；`git diff --check` 通过。当前 shell 无法连通本地 5173/8000 端口，页面级实际效果需重启本地前后端后验证。

## 2026-09-11：Piancton Agent 可见找图流程（D302）

- 背景：用户参考在线数据库/Agent 的体验，希望右侧机器人不要再像普通悬浮客服，也不要在输入框上方放固定建议按钮；当用户输入一段卖点表达或找图需求时，应先用可见流程判断“用户是不是在找某个卖点下的素材”，必要时询问确认，再继续进入找图或解释。
- 实现：`AssetAgentWidget` 移除输入区上方的 suggested questions/chips，保留对话流、输入框和发送按钮；等待态文案改为“判断当前需求：找图 / 解释 / 销售回复”“对照六大体系、核心卖点和素材事实”“组织成可确认、可继续推进的回答”。后端 `asset_agent_chat` system prompt 增加统一业务机器人流程：找图/找素材/配图需求先判断核心卖点和体系，信息不足时先问“你是想找【卖点名】这个卖点下的素材吗”，确认后再按素材事实推荐图片。
- 边界：本轮不新增数据库迁移，不新增接口，不改变搜索主链路、知识库/向量库路由、人工 accepted 图片关系或素材排序；没有候选图片上下文时，Agent 只能说明下一步，不能编造素材名称、数量、学校案例或效果数据。
- 后续：如果要做到真正逐字流式输出，需要再单独改后端响应协议和前端消息渲染；本轮先完成“可见流程 + 结构化回答风格”的体验。

## 2026-09-11：Piancton Agent 通用化（D301）

- 背景：用户确认右侧机器人不应拆成多个分开的 Agent；产品上应是一个通用能力机器人，统一承接图片、卖点、六大体系、素材使用和销售话术等问题。
- 实现：前端侧栏标题、打开入口、图片快捷操作、默认问候、默认问题、输入占位和思考态说明统一改为 `Piancton Agent` 口径。API 中心当前模型位置和环境导入 Key 标签同步改为 `Piancton Agent：通用业务问答`。后端默认问候、空消息提示、兜底回答和模型 system prompt 从“素材库业务解释 Agent”扩展为“Piancton 通用业务 Agent”。
- 规则：Agent 仍优先使用图片/素材上下文和项目启用卖点简表；有图片就结合图片事实回答，无图片就按卖点简表做通用解释；项目没有确认的信息明确说未确认，不编造。
- 边界：底层任务名继续保留 `asset_agent_chat`，兼容 API 中心调度、历史日志、测试和现有会话接口；不新增数据库迁移，不改变搜索链路，不写入业务事实，不改人工 accepted 关系，不替代素材审核。

### 后续

- 在同一个 Piancton Agent 中接入知识库服务和 VikingDB 向量库能力，而不是新增多个入口。
- 后续可按对话意图在内部自动选择“解释卖点 / 解释图片 / 销售话术 / 体系说明”，但不在界面上拆成多个机器人。

## 2026-09-11：素材库 Agent 侧栏化（D300）

- 背景：用户希望把右下角悬浮的小机器人替换为更正式的 Agent 对话框。入口仍在素材库页面，但点击后应把右侧推出一个可长期承载知识库、向量库、图片解释、卖点解释和销售话术问答的 Agent 面板；顶部搜索框保留，未来可接入独立搜索引擎。
- 实现：`AssetAgentWidget` 支持受控打开状态，关闭时只显示轻量 `Agent` 按钮；打开时渲染为固定右侧对话侧栏。`ImageHome` 持有侧栏开关状态，桌面端打开侧栏后为右侧预留 440px，固定搜索头和瀑布流同步缩窄；移动端仍使用覆盖式抽屉，避免挤压小屏布局。
- 复用：继续复用现有后端用户私有会话、图片上下文、会话切换/删除、快捷问题、发送消息和 Asia/Shanghai 每日 00:00 清空机制；“发送到素材库 Agent”的图片快捷操作会直接打开新侧栏并带入图片上下文。
- 边界：本轮不新增后端接口、不新增数据库迁移、不改变搜索链路、图片事实、人工 accepted 关系、API 中心任务或 Agent Prompt；知识库、VikingDB 向量库、销售话术和图片解释的深度接入留到下一步在同一侧栏承载位上扩展。
- 验证：前端 `npm run typecheck` 通过；前端 `npm run lint` 通过；本地 `http://127.0.0.1:5173/` 页面树确认展开后存在“素材库 Agent 对话侧栏”，收起后只保留“打开素材库 Agent”按钮。

### 后续

- 在同一右侧侧栏内设计“问卖点 / 问图片 / 问销售话术”的交互分层。
- 将知识库服务、VikingDB 向量库和素材库 Agent 的回答能力统一接入该侧栏，而不是继续新增分散悬浮入口。
- 视用户反馈微调侧栏宽度、按钮位置和移动端覆盖方式。

## 2026-09-11：知识库主链路预算与解析收口（D299）

- 背景：用户在火山控制台测试知识库问答正常，但本地搜索表现像走了 VikingDB 向量兜底，需要确认是知识库本身未命中、链路接错、超时，还是 Key/服务配置问题。
- 排查：先用旧配置复测时火山返回 `service apikey not match`，后按用户确认的新测试 Key 覆盖保存到本地 API 中心配置。再次直接调用 `kb-service-4078ae52d3bfcb46` 后不再鉴权失败，30 秒超时，60 秒成功，实际耗时约 50 秒；知识库最终答案正确返回 `AI 拍题精学` 与 `理解出题原理/举一反三` 两个核心卖点。
- 根因：完整搜索链路虽然知识库服务配置为 60 秒，但本地 `backend/.env` 把卖点理解分支预算覆盖为 `SEARCH_SELLING_POINT_TIMEOUT_SECONDS=25`，导致搜索中知识库还未生成完就被取消；同时知识库回答解析把最终答案、思考过程和定义/参考片段一起打分，使“拍题/知识点/原理”等短泛词误带出 `AI错题本`、`AI私教答疑`、`动画精讲`、`万能解法`。
- 实现：`VikingKnowledgeServiceClient` 在 HTTP 4xx/5xx 时读取火山响应体里的 `message/error/msg` 摘要并透传。`backend/.env` 与 `backend/.env.example` 将 `SEARCH_SELLING_POINT_TIMEOUT_SECONDS` 调整为 60。`VikingKnowledgeServiceRouter` 优先抽取最终答案里的“核心卖点”段落，支持同一行和多行编号格式；核心卖点解析只使用卖点名、code 和足够具体的业务短语，不再使用 `拍题/知识点/原理` 等短泛词扩大匹配。最终答案为空时仍可回退到参考片段兜底。
- 边界：不把真实 Key 写入代码或文档；不改变 0.2 fallback 阈值、VikingDB V2 字段、图片事实源、人工 accepted 关系或前端展示结构。顶部解释 API 仍是非阻断项，超时不影响卖点与图片。
- 验证：知识库单测 60 秒成功，耗时约 50 秒；真实知识路由器输出仅为 `同步自学体系 > AI拍题精学` 与 `同步考点体系 > 举一反三`；完整搜索同句不降级，返回 5 张图，命中卖点仅为上述两个；相关 33 个后端测试通过，Ruff 通过。

### 后续

- 如希望搜索体验更快，优先优化知识库 Prompt、结果返回数量或火山知识库模型配置；当前为保证主链路正确，先接受约 50 秒的知识库主判断耗时。
- 继续治理 `search_route_explanation` 顶部解释 API 的 8 秒超时；它不阻断图片返回，但会影响黑色裁决卡解释质量。

## 2026-09-11：API 中心完整搜索链路体检（D298）

- 背景：用户要求不要继续遇到问题就局部打补丁，而要整体梳理“配置是否正确、知识库是否可用、VikingDB 是否可用、最终搜索是否真的走对链路”。之前 API 中心只能分别保存和测试外部连接，不能一次性说明整条搜索链路哪里通、哪里降级。
- 实现：新增 `POST /api/admin/api-center/external-connections/search-chain/diagnose`，同一句测试话术依次检查 API Center 外部连接配置、卖点知识库服务、VikingDB V2 向量库和最终搜索链路。最终搜索 preview 追加各分支状态，便于看到 `vikingdb_knowledge_router`、`candidate_review`、`search_route_explanation` 等实际执行结果。同步修正旧环境导入 Key 的退役任务范围，使“搜索主 Key/搜索备用 Key”能进入当前 `search_result_recommendation_reason` 任务候选；人工旧 Key 不自动迁移。
- 前端：API 中心“知识与数据库”页顶部新增“完整搜索链路体检”面板，支持输入测试话术并一键运行。总状态分为 OK、降级可用、失败：最终能产出卖点和图片但存在上游失败时显示“降级可用”，不再误报全通。
- 边界：不写入真实 API Key 到代码或文档；不改变本地图片事实源、人工 accepted 关系、fallback 阈值、fallback top1 策略、短泛词拦截或素材排序。顶部解释 API 是非阻断加分项，硬等待上限收口到 8 秒；失败或超时后使用稳定默认说明，不影响卖点判断和图片返回。
- 验证：后端 Ruff 通过；后端定向测试 `11 passed`；前端 `npm run typecheck` 通过；`npm run generate:api` 已同步 OpenAPI 类型；`git diff --check` 通过。本地页面复测 `洋葱拍题精学习`，体检显示“降级可用”：VikingDB 知识路由 OK，命中 `同步自学体系 > AI拍题精学` 并返回 2 张已审核素材；知识库服务单测仍 12 秒超时，顶部解释分支按 8 秒上限超时降级，不再拖到 36 秒以上。

### 后续

- 治理知识库服务超时：优先在 API 中心调高知识库超时或优化知识库 Prompt/召回参数，目标是“知识库服务”单测从失败变为 OK。
- 治理顶部解释 API：当前 `search_route_explanation` 分支连接失败，需检查 API 中心模型 Key/地址/网络；该分支失败不应阻断图片返回，但会影响黑色裁决卡解释的实时生成质量。
- 用同一体检面板补测短词、长句、多卖点句和无效话术，把每类表现固化成后续回归用例。

## 2026-09-11：VikingDB V2 字段热修（D297）

- 根因：本地 API 中心配置已开启知识库服务与 VikingDB 向量 fallback，且 fallback 阈值为 `0.2`、卖点数为 `1`；但正式搜索链路默认请求了 `source_id/channel/status` 等字段。当前 V2 知识索引不包含 `source_id`，火山返回 400，导致原本分数高于阈值的 top1 卖点无法进入后续本地图库召回。
- 实现：`VikingDBClient.search_text()` 默认 `output_fields` 收窄为 `doc_id/doc_type/concept_code/search_text`，与 API 中心连接测试和当前知识索引保持一致；`SearchKnowledgeFallbackRouter` 兜住知识库服务调用异常，知识库服务 403、超时或格式异常时仍可继续调用 VikingDB 向量 fallback。
- 边界：没有改变 fallback 阈值、卖点数、多卖点判断、短泛词拦截、本地图片事实源或人工 accepted 关系；非业务问题仍可被知识库服务终止为无可靠卖点，不强行返回随机图。
- 配置：本地 API 中心已写入知识库服务与 VikingDB V2 向量库的连接配置，真实 Key 只保存在本地运行配置/数据库，不进入代码和文档。
- 验证：后端定向测试 `10 passed`；真实火山接口验证同一 payload 在收窄字段后成功返回 `AI拍题精学`，分数约 `0.618`；本地页面搜索“洋葱拍题精学习”返回 2 张已审核素材，顶部展示“结果 / 判断 / 定义”，不再显示 403/400 降级错误。
...
- 根因：D285 已把长图详情改成框内滚动，但详情页仍在打开时直接渲染 `contentUrl` 原图。对超长竖图来说，首屏需要下载和解码完整原图，导致详情页进入速度慢。
- 实现：长图详情默认使用 `thumbnailUrl` 作为主预览源，继续保留固定高度预览框和框内纵向滚动；预览区增加“预览图加载更快 / 查看原图”控制，用户主动点击后才切换到 `contentUrl` 原图。
- 影响范围：只改前端详情页加载策略；下载菜单仍走原有原图/尺寸下载逻辑，不增加下载次数。不改数据库 schema、不改原图文件、不改缩略图生成规则、不改身份码、渠道、素材组关系、搜索或排序。
- 验证：`npm run typecheck`、`npm run lint`、`npm run build` 通过。

### 后续

- 服务器更新前端镜像后，用超长竖版合作案例图进入详情页，确认默认打开更快；如需看像素级清晰度，再点击“查看原图”。
- 如果首页瀑布流继续因素材量和长图数量增加而变慢，再单独推进列表虚拟化或更小首屏预览档位；本次仅处理详情页首开原图加载。

## 2026-09-10：长图上传像素上限（D290）

- 根因：当前上传校验只有一个 `MAX_IMAGE_PIXELS=80000000`，对普通图片是合理安全阀，但对超长竖图/运营长图会误伤。用户上传的长图如果总像素超过 8000 万，即使文件大小未超过 100MB，也会被后端拒绝。
- 实现：新增长图专用上限 `MAX_LONG_IMAGE_PIXELS=160000000` 与 `LONG_IMAGE_MIN_ASPECT_RATIO=3.0`。上传校验先执行普通 8000 万像素上限；超过普通上限时，只有长宽比达到 3:1 且总像素不超过 1.6 亿的图片才允许继续。错误提示改为包含实际像素数和命中的普通/长图上限。
- 影响范围：主图上传、追加版本、替换主图、本地存储和 TOS 存储均使用同一校验；不改数据库 schema、不压缩原图、不改变缩略图清晰规则、身份码、搜索、渠道或素材关系。
- 验证：`python3 -m compileall` 相关后端文件和测试文件通过；相关 Ruff 通过；`git diff --check` 通过。新增后端用例覆盖“长图可超过普通像素上限”和“非长图仍受普通上限限制”。本机 pytest 未执行成功：`backend/.venv` 为 Python 3.9，不支持项目当前 `str | None` 语法；系统 Python 3.11 未安装 pytest。

### 后续

- 服务器更新后，如果 `.env` 没有显式设置 `MAX_LONG_IMAGE_PIXELS`，会使用默认 1.6 亿；如需更大长图，优先调整 `MAX_LONG_IMAGE_PIXELS`，不要直接无限放大普通图上限。

## 2026-09-10：上传渠道跨设备同步（D289）

- 根因：D286 已让首页顶部渠道筛选读取后端 `/api/images/channels`，但上传弹窗和详情页“业务筛选信息”仍只合并默认渠道、当前图片渠道和当前浏览器 `localStorage` 里的自定义渠道。因此同一个账号换设备或换浏览器后，未保存在本机缓存里的渠道不会出现在上传选项中。
- 实现：前端抽出可复用 `useImageChannelOptions()`，上传弹窗在打开时读取后端全库已使用渠道；详情筛选面板把后端渠道与当前图片已有渠道合并。`添加渠道` 的本机即时体验保留，新渠道只要随图片保存，就会通过后端渠道接口同步到其它设备。
- 影响范围：只改前端渠道选项来源；不改数据库 schema、不改上传接口、不改业务端添加权限、不改搜索、排序、图片文件或身份码。
- 验证：`npm run typecheck`、`npm run lint`、`npm test -- AssetFilterMetadataPanel channelIntent --run` 通过。

### 后续

- 服务器更新后，在 A 设备把图片渠道保存为 `合作案例/小学洋葱优势/初中洋葱优势` 等，再用 B 设备刷新并打开上传弹窗，应能看到相同渠道。

## 2026-09-10：详情页相关图片热修（D288）

- 根因：D287 给 `ImageRepository.list()` 增加 `channel` 参数后，详情页相关图片服务里的兜底列表调用仍使用旧参数顺序。打开图片详情时，后端在取相关图片阶段抛出异常，前端把失败详情误显示为“图片不存在”。
- 实现：`RelatedImageService._candidate_images()` 的兜底查询显式传入 `channel=None`，保持详情页相关图片不受当前渠道筛选影响。
- 影响范围：仅修复详情接口的后端调用参数；不改数据库 schema、不改图片文件、不改搜索、渠道筛选、身份码、下载或素材关系。
- 验证：确认仓库内 `ImageRepository.list()` 调用点已同步；`python3 -m compileall` 相关后端文件通过；相关后端 Ruff 通过；`git diff --check` 通过。

### 后续

- 服务器拉取本次提交并重建 backend 后，刷新页面再从任意素材卡进入详情，应不再出现“图片不存在”。

## 2026-09-10：渠道筛选先于分页（D287）

- 根因：首页浏览列表原来先按最近上传分页取 12 张，再在前端执行渠道筛选。持续新增同类图片后，较早上传的《洋葱是怎样让学生理解抽象概念的？》会被挤出当前已加载页；图片仍在数据库中，删除前面的数学/语文素材后又回到第一页，所以表现为偶发“隐形消失”。
- 实现：`/api/images` 新增 `channel` 查询参数，后端按渠道 token 匹配后再执行游标分页；前端首页点击渠道按钮时把选中渠道传给列表查询，分页和无限滚动都在该渠道结果集内继续。多渠道值兼容 `、`、英文逗号、中文逗号、斜杠和全角斜杠分隔。
- 影响范围：只改变普通首页手动渠道筛选的列表分页边界；不改数据库 schema、不改卖点搜索/语义搜索、不改排序字段、不改下载、身份码、素材关系或业务事实。前端仍保留二次过滤，防止旧缓存或异常数据混入。
- 验证：`npm run typecheck`、`npm run lint`、前端全量 `npm test -- --run` 19 files / 64 tests passed、`npm run build`、`python3 -m compileall` 相关后端文件、后端相关 Ruff、`git diff --check` 均通过。新增后端回归 `test_channel_filter_is_applied_before_pagination`，本机 pytest 环境仍缺少可用 3.10+ pytest，待容器/服务器测试环境执行。

### 后续

- 服务器更新后，在业务端点击 `小学洋葱优势` 或对应渠道，不断新增其它图片后，确认《洋葱是怎样让学生理解抽象概念的？》仍会在该渠道结果中通过分页稳定出现。

## 2026-09-10：业务端渠道筛选同步（D286）

- 根因：设计师端“添加渠道”与渠道管理话术仍保存在当前浏览器本地；业务端账号、另一台电脑或另一个浏览器不会天然拥有同一份本地渠道目录。同时首页顶部筛选不能只依赖当前分页已加载图片，否则自定义渠道可能因为不在前 12 张图片里而不出现。
- 实现：后端新增只读 `/api/images/channels`，从所有未删除图片的 `channel` 字段汇总当前素材库真实已使用渠道，并拆分 `官网大图、手机端大图` 这类多渠道值。前端首页新增渠道查询，将全库渠道与默认渠道、搜索结果动态渠道合并后展示；点击自定义渠道不再被本地渠道目录校验清空。
- 权限与边界：所有已登录用户可读取渠道列表；业务端仍不显示“添加渠道”。设计师端本地新增渠道的即时体验保留，只有渠道保存到图片后才成为业务端可见的真实筛选项。本轮不新增数据库表，不改卖点搜索、人工 accepted 关系、渠道意图 Skill 或图片下载。
- 验证：`npm run typecheck`、`npm run lint`、`npm run build` 通过；`npm test -- searchResultFilters channelIntent --run` 为 2 个文件 21 passed；相关后端文件 `python3 -m compileall` 通过。后端定向 pytest 在本机未执行成功，原因是 `backend/.venv` 为 Python 3.9 不支持项目当前 `str | None` 语法，系统 Python 未安装 pytest；新增测试已写入 `test_security_and_images.py`，待容器或 3.10+ 后端测试环境执行。

### 后续

- 服务器更新后，用设计师账号确认某张图片渠道含 `合作案例/小学洋葱优势/初中洋葱优势/品牌通用类`，再切业务账号刷新首页，顶部应出现相同渠道并可筛选。
- 如果未来需要“新增渠道但暂时没有任何图片也让所有人看到”，再把渠道话术目录从浏览器本地迁入后端正式渠道字典；本次先不扩大为管理系统迁移。

## 2026-09-10：长图详情页窗口化预览（D285）

- 根因：设计师详情页左侧主图按原图高宽比撑开卡片，1342×3297 等长竖图会把整个详情页变成很长的页面，右侧信息区和后续模块被迫跟随外层页面滚动。
- 实现：当图片高宽比大于 `1.65` 时，详情页主预览切换为内部滚动窗口；预览框使用视口高度上限，图片按框宽展示并在框内纵向滚动，添加 `overscroll-contain` 降低滚动串到外层页面的概率。非设计师嵌入式详情同样复用该判断；横图和普通竖图继续使用原有居中完整适配。
- 影响范围：只改前端 `ImageDetail.tsx` 的展示容器；不改原图、缩略图、下载、素材组版本关系、身份码、搜索或数据库。
- 验证：`npm run typecheck`、`npm run lint`、`npm run build` 通过。

### 后续

- 服务器更新前端镜像后，用 1342×3297 的合作案例竖版图确认左侧主预览在框内滚动、右侧信息栏不再被长图拉高。
...
- 身份码收口为每张活动图片一个 `identity_code`。移除素材组 `asset_code`、图片 `version_code`、永久身份登记表和分享链接入口；上传主图、延展图和替换主图都各自获得一个码。
- 删除图片时清空身份码，精确搜索和管理员身份码页立即不可见；恢复时分配新身份码，不展示或保留删除历史。管理员页面只提供活动图片搜索、复制身份码和查看图片。
- 新增迁移 `20260909_0035_single_image_identity_codes.py`。SQLite 空库完成 `upgrade head → downgrade 0034 → upgrade head`；40 张活动图片的数据副本升级后全部获得唯一身份码，旧登记表已移除。真实本地数据库未在开发过程中改写。
- OpenAPI 类型已重新生成；新增卖点管理页面和身份码管理页面前端测试，并重写身份码后端专项。
- 验证：前端两个新测试通过，TypeScript、ESLint 和 production build 通过；后端身份码、文档一致性、安全与 TOS 专项最终 43 passed，相关 Ruff 与 Pyright 通过。额外纳入 Phase 5 文件时仍仅有两项已退役图片分析接口旧状态码预期失败，与本轮无关。

### 后续

- 服务器更新时先执行 Alembic `upgrade head`，再重建 backend/web；确认现有活动图片各有一个码、删除后管理页立即少一条、恢复后出现新码。
- 从卖点管理依次切换六个体系，核对右侧卖点归属与业务目录一致。

## 2026-09-09：长图清晰预览（D281）

- 根因确认：后端原缩略图把所有图片限制在 `640×640` 内，长图高度到 640px 时宽度会按比例降到很低；前端瀑布流再按整列宽度显示，导致图片中的文字和细节被放大后模糊。原图文件本身没有被这一步覆盖。
- 新上传缩略图改为优先保证最多 640px 的预览宽度，高度按原比例保留并设置 12 倍宽度安全上限；使用 Lanczos 缩放，JPEG 调整为 quality 90、4:4:4。示例 1200×3600 长图由旧规则的 213×640 改为 640×1920。
- 兼容已有素材：`GET /api/images/{id}/thumbnail` 检查竖图缩略图实际宽度；旧缩略图宽度不足时返回原图及正确媒体类型，不要求重新上传，也不增加下载计数。TOS 场景会先释放已下载的旧缩略图临时文件，再读取原图，响应完成后继续清理原图临时文件。
- 无数据库迁移；不修改原图、素材名称、素材组/版本关系、渠道、画面风格、场景图、人工 accepted 卖点关系、搜索召回或下载文件。
- 修改：`backend/app/services/storage_service.py`、`backend/app/services/image_service.py`、`backend/app/api/v1/images.py`、两份存储/图片接口测试，以及素材库运行规则。
- 验证：`test_security_and_images.py + test_tos_storage.py` 共 36 passed；相关 Ruff 通过；修改服务与 API 的 Pyright 为 0 errors；`git diff --check` 通过。Docker 当前未启动，服务器真实长图页面与流量尚待更新后验收。

### 后续

- 服务器重建 backend 后刷新页面，用同一张历史长图确认无需重传即可变清晰；再上传一张新长图，确认缩略图对象短边宽度符合新规则。
- 观察 TOS 出网与后端临时磁盘；只有极端长图或历史低清图会回退原图。若此类素材数量明显增加，再增加独立的多档预览缓存，不降低清晰度。

## 2026-09-09：名称释放、版本对称浏览与筛选信息回编（D280）

- 修订 D070：图片进入回收站即释放显示名称；活动图片继续大小写/首尾空格归一化唯一。若旧图恢复时原名已被新图复用，恢复流程自动分配下一组三位序号，并在旧图仍是素材组封面时同步组标题。当时保留的两级永久身份码规则现已由 D282 取代。
- 新增迁移 `20260909_0034_release_deleted_image_titles.py`：名称唯一索引改为仅约束 `deleted_at IS NULL`，移除 `image_title_reservations`；降级会先为冲突记录稳定编号，再恢复旧全局唯一与占用表。SQLite 空库完成 `upgrade head → downgrade 0033 → upgrade head` 往返。
- 详情页版本区按当前路由图片排序和高亮，标签收口为“当前查看 / 素材组封面 / 关联版本 / 历史版本”；点击任何横版或竖版都会切换上方主视图。`primary_image_id` 继续负责素材组默认搜索封面，不再与当前查看状态混淆。
- 新增 `PATCH /api/images/{image_id}/filter-metadata` 和详情“业务筛选信息”面板。渠道修改当前版本；画面风格、场景图修改整个素材组；保存后刷新当前版本与素材组封面的搜索/向量派生同步点。现有卖点关系面板和 D279 整组替换接口保持不变。
- OpenAPI 类型已重新生成。新增后端名称释放/恢复冲突、筛选信息更新与索引同步回归；新增前端版本双向切换测试。
- 验证：本轮后端专项 4 passed；`test_security_and_images.py + test_image_titles.py + test_phase5_endpoints.py` 为 33 passed / 2 failed，失败仍是已退役图片分析接口旧预期；相关 Ruff、Pyright 通过。前端 TypeScript、ESLint、5 个相关测试和 production build 通过。Docker 当前未启动，未做常驻页面浏览器验收或服务器部署。

### 后续

- 服务器更新时先执行 Alembic `upgrade head` 再重建 backend/web；用一组真实横版/竖版从两张卡分别进入详情，确认双向切换和筛选信息回编。
- 在回收站用真实同名素材验证“删除原图 → 重传原名 → 恢复旧图自动编号”；身份码生命周期按 D282 的删除清空、恢复新分配执行。

## 2026-09-08：上传多卖点关系与详情收口（D279）

- 上传主图可一次选择一个“主要表达”和多个“同时可以支持”，发布后直接写入人工 accepted `expresses/supports` 关系；批量上传共用本次关系。
- 新增 `PUT /api/asset-groups/{group_id}/concept-links`，整组替换人工卖点关系，限制最多一个主要表达，并支持详情页一次新增、改角色和取消旧关系。
- 详情页把“业务表达层级”和“业务卖点关系”合并为“卖点关系”；移除退役图片分析留下的历史画面信息、语义总结和上传后的素材话术提示。
- 图片卡片悬浮快捷操作移除无交互的中心光点，保留轨道和真实操作按钮。
- 无数据库 schema 迁移；不改六体系、16 卖点、搜索路由或 TOS 存储规则。
- API 中心后台维护已经随 backend 自动运行：默认首次延迟 5 分钟，以后每 360 分钟抽检一次，并清理过期调用/健康日志；不承担自动部署、备份或图片业务关系生成。
- 验证：新增后端整组替换接口专项 `1 passed`；Phase 5 文件合计 `10 passed / 2 failed`，两项失败均仍是退役图片分析端点旧预期；前端多卖点组件测试、TypeScript、ESLint 和生产构建通过；本地浏览器已检查上传弹窗与详情页最终布局，接口类型已重新生成。
...
# 2026-09-07 D276 API 配置布局与任务选项

- 验证：前端 TypeScript、ESLint、13 文件 / 54 测试及生产构建通过，`git diff --check` 通过；服务器浏览器更新后的实际布局仍待现场确认。
- 修改 `client/src/tailwind-theme.css` 和 `client/src/pages/AdminApiCenter/AdminApiCenter.tsx`：去除退役任务筛选项；固定 API 配置使用自适应网格，开关与等待参数横跨全部列，备用选择弹层限制在视口内；文案明确一个主 API 加多个备用，以及自动模式的候选分配方式。
- 沿用后端主备与自动调度协议，不改业务事实、不新增数据库迁移、不删除历史调用记录。
- 飞书登录仍未实现；用户可先通过现有用户管理创建 business 账号。后续接入需公司自建应用及企业范围校验，首次自动创建普通业务账号，不自动赋予管理员权限。
- 服务器更新仅需拉取本分支并重建 web；已有 `.env`、API 库存和账号不变。
- 2026-09-09 VDB-V2 原文关系检索实验准备：用户明确要求以六体系原文为主，解决“图片/证据表达能反推出卖点但线上数据库检索不到”的问题。本轮新建独立 `datasets/vikingdb_knowledge/v2_source_grounded/` 数据包，不改生产 `piancton_search_knowledge_clean`、项目 VikingDB 配置、图片人工关系或图片索引。数据按 `system / selling_point / proof_point / evidence_point` 四层生成 136 条记录（6/16/55/59），普通实验检索允许后 3 层参与，再按 `concept_code` 汇总父卖点；库存缺口、素材状态和审核说明仅保留审计快照，不进入检索正文。原脑图路径从 `sourcePaths` 逐条解析；“专家身份说明”两条分支和“培优课一题多解”原文分支与后续证明点治理映射的两处差异均显式保留，不把后续治理关系冒充原文。同步培养学校落地案例（含岳阳二十中等）按原图“万能解法/学校落地实证”补充并标记未独立核验；未凭“北京市合作案例”标题制造北京到唯一卖点的关系。增加 84 条独立评测用例、8 项本地测试、V2 写入目标保护和冲突拒写。当前本地测试全部通过，Ruff 和 diff check 通过。线上阶段未执行：现有数据 API Key 对旧库统计和普通检索均返回 `403 AccessDenied`，故未上传 V2、未生成线上准确率；待北京区域控制台新建目标集合/索引、复制旧库 embedding 配置并补充该 Key 权限后，使用包内 `run_live.py` 对旧库只读、V2 写入与同批对比检索。 |

# 2026-09-14 D342 Agent 当日连续记忆与每日全量清空

- 用户澄清最终口径为“当天有记忆、跨天全清空”：手动输入和推荐问题都携带同一会话的最近上下文，点击“新对话”立即创建空白记忆窗口。
- 后端恢复 Asia/Shanghai 自然日边界；每天 00:00 后该用户首次列表、新建、更新、删除或发送 Agent 消息时，删除前一自然日的全部会话及关联消息；跨夜页面使用旧会话 ID 发送时自动进入新的空白会话，不继承昨日内容。
- Agent 顶部容量圆环改为“今日记忆”，按最近最多 24 条用户/助手消息、单条最多 1200 字、总计最多 12000 字显示占用；70% 后变黄，90% 后变红。
- D341 的跨天持久实现在云端部署前撤销；本地数据库已降回 `20260913_0040 (head)`，可空 `expires_at` 迁移已删除，本轮无新数据库迁移。
- 验证：Agent 专项 12 项通过；Agent/AI Search 行为闭环/Skill/文档联合回归 42 项通过、4 项按环境跳过；前端 31 个文件共 87 项测试通过；TypeScript、ESLint、Ruff、OpenAPI 类型重生成、Docker 生产构建和四服务健康检查通过。
- 聊天仍按用户私有隔离，不写入六大体系、16 个核心卖点、人工 accepted 关系、搜索排序或火山数据集。

# 2026-09-14 D343 图片右侧纵向快捷操作条

- 按用户截图移除图片卡片的中心点、轨道和弧形圆盘，改为右侧 `10px` 内边距、`32px` 高、`6px` 间距的右对齐白色胶囊纵向操作条。
- 普通业务卡片从上到下显示“下载、复制身份码、收藏到画板、喜欢、发送到 Agent”；搜索结果继续在同一列保留“加入/移出多选”，多选导出功能未删除。
- 桌面端仍仅在图片卡片 hover 或键盘 focus-within 时显示；触摸和粗指针设备常显。外层继续永久透传指针，只有按钮与尺寸选择器接收点击，不会重新遮住图片详情或相邻卡片。
- 修改文件：`client/src/pages/ImageHome/RadialActionMenu.tsx`、`client/src/pages/ImageHome/RadialActionMenu.test.tsx`、`client/src/tailwind-theme.css`、总纲、项目日志与 UX 规范。
- 数据迁移：无；不改变素材、操作回调、权限、搜索、排序、推荐、Agent、六大体系、16 个核心卖点、人工 accepted 关系或火山数据集。
- 验证：快捷操作专项 `1 passed`；前端完整 `31` 个测试文件、`87` 项全部通过；TypeScript、ESLint、Docker 前后端生产构建通过；本地浏览器业务账号测量确认轨道不存在、默认透明、外层 `pointer-events: none`，五个动作右边线一致，按钮高度均为 `32px`、纵向起点依次为 `195/233/271/309/347px`，步距一致为 `38px`。
