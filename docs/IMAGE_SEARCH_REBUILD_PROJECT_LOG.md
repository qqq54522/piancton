...
- 移除业务用户从注册次日起每天第一次进入时强制显示的“昨日使用反馈”弹窗。业务用户完成新人引导后可以直接进入素材库，不再因日期、刷新、重新登录或换设备被反馈表阻断。
- 前端删除 `DailyFeedbackDialog`、布局触发条件、Auth Context 完成动作和对应 API 调用；右上角账号菜单中的用户主动“提交反馈”入口保持不变。
- 后端 `UserRead` 不再返回 `dailyFeedbackRequired`，删除 `/api/auth/daily-feedback/complete` 接口和完成服务。旧前端拿不到强制标记，也不会继续弹窗；当前前端不再调用已删除接口。
- 历史迁移 `20260916_0043` 和 `users.daily_feedback_completed_on` 可空旧列保留，避免对现有数据库做没有必要的破坏性迁移；运行时不再用该字段判断是否弹窗，也不再返回或写入该状态。本轮无新迁移，不改账号权限、素材、搜索、推荐、Agent、六大体系、人工 accepted 关系或火山数据集。
- 验证：后端注册专项 14 项通过；OpenAPI 实际输出确认强制反馈接口和字段均不存在；前端 37 个测试文件 100 项、TypeScript、ESLint、生产构建通过；后端改动文件 Ruff、Python 编译检查和 `git diff --check` 通过。生产构建仅保留既有大 chunk 提示，无构建失败。

## 2026-09-19：当前版复刻文档收口

- 根据外部飞书导出的《卖点智库 v1.0》重新整理 `docs/SELLING_POINT_KNOWLEDGE_REPLICATION_GUIDE.md`。新版本删除项目价值、画板原型和排期等不属于当前技术复刻的内容，改为说明当前代码、AI Search 单入口、数据库事实源、同步步骤和验收顺序。
- 向量数据库/Embedding、独立知识库与 VikingDB 路由、旧 API 中心/ModelProvider、上传图片分析和旧素材话术生成统一作为历史研发路线与踩坑背景，不再作为复刻时需要启动或配置的组件。Meilisearch 保留为可选增强索引，不是事实源。
- 另新增 `docs/SELLING_POINT_KNOWLEDGE_RESEARCH_LOG.md`，把原始文档中的旧路线、决策原因、详细踩坑和后续飞书重写建议单独承接，避免当前复刻步骤被历史组件污染。无代码、数据库迁移或运行时行为变更；未修改外部原始导出目录。文档核对依据为当前总纲、护栏、架构文档、Skills 索引、AI Search 同步脚本和 Docker 配置。后续文档验收以当前代码和总纲为准。

## 2026-09-20：研发日志和复盘扩充为详细底稿

- 用户确认研发日志也需要达到复刻指南的详细程度，不能只列技术路线和几个坑。本轮扩充 `docs/SELLING_POINT_KNOWLEDGE_RESEARCH_LOG.md`，新增从最初标签方案、向量/Embedding、六大体系、独立知识库、VikingDB fallback 到 AI Search 单入口的分阶段过程；每阶段补充原始假设、验证方法、现场现象、根因、保留/淘汰内容和后续影响。
- 继续补充业务概念、人工 accepted 关系、延展尺寸、搜索问题记录、Agent 图片卡、视觉桥、部署丢数据、测试记录和外部服务验收的详细排查手册，并提供可直接继续追加的研发问题模板。复刻指南只负责操作，研发日志负责历史原因和复盘，二者职责分开。
- 无代码、数据库迁移或运行时行为变更；本轮只更新研发日志和项目日志。`git diff --check` 已通过。

## 2026-09-20：补齐火山 Viking AI 搜索控制台复刻步骤

- 用户指出复刻指南只写了 `.env` 参数，没有写小白如何从火山引擎控制台开始创建 Viking AI 搜索资源。本轮把指南第 8 节改为实际操作顺序：登录并开通 Viking AI 搜索、创建 API Key、创建图片物品数据集、创建知识数据集、创建应用并绑定数据集、发布普通搜索场景、配置并发布 `chat_search`、可选创建推荐和用户行为数据集，再将控制台结果逐项填入环境变量。
- 同时补充控制台对象和项目变量的对应表、应用/数据集/场景 ID 的区别、普通搜索与 `chat_search` 的首次控制台体验测试、为什么先做普通搜索再做视觉问图，以及公网图片桥的检查。官方参考入口保留在指南中；控制台菜单名称可能变化，不能把路径写死成某个旧截图。
- 无代码、数据库迁移或运行时行为变更；本轮只更新复刻文档和本日志。`git diff --check` 需通过后再交付。

## 2026-09-20：完整归档最初立项文档中的历史技术经验

- 用户补充确认：最初导出的文档不能只作为提炼素材，里面的旧方案、旧代码说明、旧版复刻过程、研发日志、踩坑、验证和后续变更记录都是经验，必须完整进入研发日志/复盘，方便以后重新写入文档和追溯原因。
- 已把 `/Users/yangcong/Desktop/卖点智库文档_Markdown导出/【立项】卖点智库v1.0版本.md` 中从“系统蓝图与核心资产”开始的技术材料，完整追加到 `docs/SELLING_POINT_KNOWLEDGE_RESEARCH_LOG.md` 的第 24 节。归档内容包括旧技术栈、搜索流程、业务意图分层、卖点话术治理、API 调度中心、身份码、向量搜索边界、旧版小白复刻步骤、旧版研发日志、踩坑复盘、测试验收和历史变更经验。
- 该节明确标记为历史材料：保留原文上下文，不把已退役的 API 中心、独立 Embedding、旧知识库路由、旧模型调用重新列为当前复刻步骤；当前复刻仍以 `SELLING_POINT_KNOWLEDGE_REPLICATION_GUIDE.md` 为准。按用户要求没有归档项目价值、画板占位和排期内容。
- 本轮只修改文档，无代码、数据库迁移或运行时行为变化；`git diff --check` 通过。

---

## 2026-09-16：渠道分类定点复制（D373，本地完成）

- D371 只能把来源渠道的整棵目录复制到目标渠道最外层，无法处理“只要某个下级分支”或“放进目标已有分类”的实际整理需求。本轮保留整渠道复制，并增加来源分类和目标上级两个独立选择。
- 在左侧目录树选中任意分类后可点“复制到”；工具会冻结该来源路径，复制它及全部下级，不带来源父级和兄弟目录。目标侧先选另一个渠道，再选该渠道的任意现有分类或最外层。目标同父级已有同名目录时复用，继续递归补齐缺少部分，重复操作不制造重复目录。
- `POST /api/channel-folders/folders/copy` 增加可选 `sourceFolderId` 与 `targetParentId`。Backend 校验两个 ID 分别属于声明的来源/目标渠道，只写 `channel_folders`；没有读取或修改图片目录绑定，因此图片、渠道事实和未归类状态保持原样。同渠道复制继续拒绝，避免把一个分支复制进自身形成含糊结构。
- 无数据库迁移；不改素材、搜索、推荐、Agent、六大体系、人工 accepted 关系或火山数据集。修改后本地 Backend/Web 已重建，PostgreSQL 和 Meilisearch 保持原数据卷。
- 验证：重建后后端渠道目录专项 4 项通过，覆盖整渠道幂等合并、图片不跟随、选定子树复制到指定目标父级及排除来源父级/兄弟目录；前端管理页与目录组件 2 文件 4 项通过；TypeScript、ESLint、生产构建、Ruff、`git diff --check` 均通过；四个本地服务健康，`/health/ready` 返回 ready。云端未部署。

---

## 2026-09-16：渠道分类整层删除（D372，本地完成）

- 原删除入口只在分类被选中后显示，且后端只允许删除没有下级、没有图片的空分类；一旦上层建错，用户必须逐层处理，实际无法直接撤销整棵错误结构。
- 每一行分类右侧现在直接显示删除按钮，确认弹窗会说明父分类、下级分类数量和图片处理方式。确认后删除该分类及所有后代目录；图片文件和渠道归属不变，只移除这些目录绑定并回到对应渠道“未归类”，可以重新整理。
- Backend 在单一事务内计算完整子树，先删除受影响的 `image_channel_placements`，再从最深子目录向父目录删除 `channel_folders`；审计记录删除目录数和回到未归类的图片数。无数据库迁移，不改图片、搜索、推荐、Agent、业务概念关系或火山数据集。
- 验证：后端渠道目录专项 3 项通过，覆盖五层目录、父级删除、图片仍在原渠道且回到未归类；前端渠道专项 3 项通过，覆盖删除入口、下级数量提示和显式确认；TypeScript、ESLint、生产构建、Ruff 与差异检查通过。云端未部署。
