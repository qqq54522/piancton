...
更新时间：2026-09-16
当前范围：Phase 0～Phase 6；D349～D366 AI Search 单入口、自由问答 Agent、渠道目录与渠道归属找图、卖点找图返卡、本地问图地址校验、一次性图片附件、连续问图与历史图片回指、记忆展示简化
当前状态：D366 新人引导与私有账号头像已在本地完成并通过完整回归，云端未部署；D365 已在本地完成：业务端 Agent 卖点找图按人工已确认关系返回可点击的当前素材卡，完整回归与浏览器实测通过，云端未部署；D364 已在本地完成：同名素材卡与聊天续接，完整回归和独立新版浏览器实测通过，云端未部署；D363 已在本地完成：明确渠道找图按本地已发布图片归属核对素材卡，Agent 专项通过，本地 Backend 已更新，云端未部署；D362 已在本地完成：连续发图与自然回指此前图片，完整前后端回归通过，本地服务已更新，云端未部署；D361 已实现最新待发图自动切换；D360 已删除不准确的“今日记忆”百分比与 Piancton 固定窗口，同会话追问由火山 `session_id` 承接；D358 已阻止不可达图片地址导致的假看图，真正视觉问答仍需公网图片服务地址。下文 D330～D346 是回退前的历史实施记录，当前事实以总纲 D349～D365 为准。
