...
1. 标题精确命中和人工确认的卖点名称/公共话术；
2. 人工新增或已采纳 AI 的当前素材独有话术；
3. 图片摘要、OCR 和客观语义；
4. AI 客观内容标签与待审核概念。

数据库召回用自动化测试固定公共话术 `0.95`、已确认素材独有话术 `0.90`、客观内容标签 `0.80` 的相对权重；Meilisearch 使用相同字段顺序。AI 待审核素材话术不进入这两个高优先级入口，只将其原始 AI 语义表达用于 Semantic Profile、Embedding 和 Reranker；拒绝或由维护者删除后从直接召回与语义投影一并排除。Embedding 用于扩大模糊语义候选，Reranker 只在存在多张候选时重排，不覆盖体系硬过滤和人工排除关系。
