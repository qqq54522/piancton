...
数据库召回用自动化测试固定公共话术 `0.95`、已确认素材独有话术 `0.90` 的相对权重；Meilisearch 使用同样的人工信号优先顺序。AI 待审核素材话术不进入这两个高优先级入口，只将原始 AI 语义表达用于低优先级 Semantic Profile、Embedding 和 Reranker；拒绝或由维护者删除后从直接召回与语义投影一并排除。Embedding 用于扩大模糊语义候选，Reranker 只在存在多张候选时重排，不覆盖体系硬过滤和人工排除关系。
