...
```text
本地 0～4 个候选卖点理解 ─┐
数据库图片/素材短语召回 ──┼─> 候选融合 ─> 卖点关系优先 ─> 体系过滤 ─> 素材组折叠 ─> 一次可选 Reranker ─> 人工关系保护
Meilisearch 关键词召回 ───┤
Embedding 语义召回 ──────┤
复杂查询模型理解 ─────────┘
```
...
- 结果顶部展示本次识别出的一个或多个候选卖点，可在当前结果内缩小；
- 卡片右下角展示本次查询命中的卖点；该标签由查询候选与素材已确认关系动态求交，不是新建的图片固定标签；
- 单结果“就是这张/不相关”反馈都记录查询、图片和素材组，只用于搜索优化，不修改发布审批或人工概念关系。

当前文字搜索优先级：

1. 高置信候选卖点及素材人工确认的 `expresses/supports` 关系；
2. 已确认卖点名称和公共话术；
3. 人工新增或已采纳 AI 的当前素材独有话术；
4. 标题、图片语义总结、画面事实和场景；
5. AI 待审核素材表达与待审核概念。

数据库召回用自动化测试固定公共话术 `0.95`、已确认素材独有话术 `0.90` 的相对权重；Meilisearch 把已确认卖点名称、公共话术和素材独有话术排在标题之前。AI 待审核素材话术不进入这两个高优先级入口，只将原始 AI 语义表达用于低优先级 Semantic Profile、Embedding 和 Reranker；拒绝或由维护者删除后从直接召回与语义投影一并排除。Embedding 用于扩大模糊语义候选，Reranker 只在存在多张候选时重排，不能覆盖体系硬过滤、人工排除关系或高置信卖点下的人工确认关系层级。
