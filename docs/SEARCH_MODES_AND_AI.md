...
```text
本地/模型查询理解 ─> 可信卖点路由 ─> accepted 卖点关系素材 ─> 素材独有话术选图 ─> 一次可选 Reranker
数据库图片/素材短语 ─┐
Meilisearch 关键词 ───┼─> 全局候选池 ─> 仅在卖点无素材或意图未消歧时兜底
Embedding 画面语义 ──┘
```
...
- 可信卖点主通道无已确认素材：启用全局图片话术、标题和画面语义候选兜底。
...
- 结果顶部只展示本次可信识别出的一个卖点或明确真多卖点，可在当前结果内缩小；未消歧候选不伪装成多个已识别卖点；
- 卡片右下角展示本次查询命中的卖点；该标签由查询候选与素材已确认关系动态求交，不是新建的图片固定标签；
- 单结果“就是这张/不相关”反馈都记录查询、图片和素材组，只用于搜索优化，不修改发布审批或人工概念关系。

当前分层搜索顺序：

1. 卖点公共话术和查询理解确定一个可信卖点，或确认一句话确实同时表达多个卖点；
2. 只在这些卖点 accepted `expresses/supports` 素材内，优先用人工新增或已采纳 AI 的素材独有话术选择具体图片；
3. 同卖点素材再由关系角色、基础召回分和一次 Reranker 补充排序；
4. 主通道无素材或意图未消歧时，才使用标题、全局图片话术、语义总结、画面事实、场景和 Embedding 兜底；
